...
export COMPUTE_IP1=$(oci compute instance list-vnics \
  ...
  --raw-output)
export private_IP1=$(oci compute instance list-vnics \
  --instance-id "${COMPUTE_OCID1}" \
...
echo "Host *" > ~/.ssh/config 
echo " StrictHostKeyChecking no" >> ~/.ssh/config
until ssh opc@${COMPUTE_IP1} 'ls'
do  
    echo "ssh running"
    sleep 10
done

ssh opc@${COMPUTE_IP1} ' sudo yum install -y git ; sudo yum install -y docker ; sudo yum install -y docker-engine ; sudo service docker start ; sudo firewall-cmd --permanent --zone=public --add-service=http ; sudo firewall-cmd --reload ; sudo docker pull gaganvamsi/tc_oracle:latest ; sudo docker run -d -p8080:8080 gaganvamsi/tc_oracle:latest'
...
export COMPUTE_IP2=$(oci compute instance list-vnics \
  ...
  --query 'data[0]."public-ip"' \
  --raw-output)
export private_IP2=$(oci compute instance list-vnics \
  --instance-id "${COMPUTE_OCID2}" \
  --query 'data[0]."private-ip"' \
  --raw-output)
sleep 30
#echo "Host *" > ~/.ssh/config 
#echo " StrictHostKeyChecking no" >> ~/.ssh/config
until ssh opc@${COMPUTE_IP2} 'ls'
do  
    echo "ssh running"
    sleep 10
done

ssh opc@${COMPUTE_IP2} ' sudo yum install -y git ; sudo yum install -y docker ; sudo yum install -y docker-engine ; sudo service docker start ; sudo firewall-cmd --permanent --zone=public --add-service=http ; sudo firewall-cmd --reload ; sudo docker pull gaganvamsi/tc_oracle:latest ; sudo docker run -d -p8080:8080 gaganvamsi/tc_oracle:latest'
...
export CREATED_B=$(oci lb backend create --load-balancer-id $LB_ID  --backend-set-name TC --ip-address ${private_IP1}  --port 8080 --weight 3 --wait-for-state SUCCEEDED)

export CREATED_B=$(oci lb backend create --load-balancer-id $LB_ID  --backend-set-name TC --ip-address ${private_IP2}  --port 8080 --weight 3 --wait-for-state SUCCEEDED)
